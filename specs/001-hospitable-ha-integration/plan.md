...
1. **The upstream API silently ignores inputs.** Five independent
   silent-ignore behaviors are CONFIRMED — a bogus calendar
   `listing_id`, an invalid `include=` name, an unrecognised parameter
   name such as `date_type=`, a `per_page` parameter ignored on
   `/channels`, and pagination URLs returned with an
...
five independent, separately discovered silent-ignore behaviors; the
most recent, `/channels` ignoring `per_page`, was found exactly as this
