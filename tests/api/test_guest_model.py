...
"""Red-phase guest identity model tests (T085, T086).

``custom_components.hospitable.api.models`` already exists, so a missing
``HospitableGuest`` surfaces as ``AttributeError`` on attribute lookup,
not as an import error. The red phase therefore pins
``raises=AttributeError`` and every import stays inside the test body.
"""
...
_RED = "TDD red phase: T085/T086 HospitableGuest is not implemented"
...
@pytest.mark.xfail(raises=AttributeError, strict=True, reason=_RED)
def test_guest_model_parses_the_four_default_fields() -> None:
    ...
    guest = models.HospitableGuest.from_api(_guest_payload(0))  # type: ignore[attr-defined]
...
def test_guest_without_a_surname_parses_with_the_surname_absent() -> None:
    ...
    guest = models.HospitableGuest.from_api(_guest_payload(1))  # type: ignore[attr-defined]
...
def test_null_guest_yields_no_guest_data_rather_than_an_error() -> None:
    ...
    assert models.HospitableGuest.from_api(None) is None  # type: ignore[attr-defined]
    assert models.HospitableGuest.from_api(_guest_payload(2)) is None  # type: ignore[attr-defined]
...
def test_reservation_model_carries_the_guest_beside_the_guest_counts() -> None:
    ...
    assert reservation.guest is not None  # type: ignore[attr-defined]
    assert reservation.guest.first_name == "Example"  # type: ignore[attr-defined]
...
def test_reservation_without_a_guest_key_parses_with_no_guest() -> None:
    ...
    assert reservation.guest is None  # type: ignore[attr-defined]
